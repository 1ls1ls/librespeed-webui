...
/*
  LOCAL MODIFICATION (librespeed-webui)
  -------------------------------------
  Upstream hardcodes the worker URL as "speedtest_worker.js", resolved relative
  to the *page*. This build serves the page from "/" but keeps the LibreSpeed
  core under "/librespeed/", so the two no longer sit side by side. Exposing the
  URL as a static property lets the app point at the worker without forking the
  file; everything else below is upstream v6.2.1, unchanged.
*/
Speedtest.WORKER_URL = "speedtest_worker.js";
...
Speedtest.prototype = {
  ...
  start: function() {
    ...
    this.worker = new Worker(
      (Speedtest.WORKER_URL || "speedtest_worker.js") + "?r=" + Math.random()
    );
